...
| --- | --- | --- |
| GPIO | General Purpose Input/Output | Dies sind die Pins, die für die Kommunikation mit anderen Geräten oder zur Steuerung von LEDs, Motoren und anderen Komponenten verwendet werden. |
| SDA | Serial Data Line | Diese Leitung wird für die serielle Datenübertragung zwischen verschiedenen Komponenten verwendet, beispielsweise für die Kommunikation mit Sensoren. |
| SCL | Serial Clock Line | Die serielle Taktleitung, die in Verbindung mit SDA für die Synchronisation der Datenübertragung verwendet wird. |
| GPCLK0 | General Purpose Clock 0 | Ein allgemeiner Zweckstaktgeber, der für zeitabhängige Anwendungen wie PWM (Pulsweitenmodulation) genutzt werden kann. |
| MOSI | Master Out Slave In | Der Master sendet Daten an einen Slave (z.B. ein anderes Gerät oder einen Sensor). |
| MISO | Master In Slave Out | Der Master empfängt Daten vom Slave. |
| SCLK | Serial Clock | Der Taktpuls für die synchronisierte serielle Datenübertragung. |
| ID_SD | ID I2C EEPROM | Wird für die automatische Erkennung von HATs (Hardware Attached on Top) verwendet. |
| PWM1 | Pulse Width Modulation 1 | Wird für die Steuerung von Motoren oder die Regelung von Helligkeit verwendet. |
| PCM_FS | Pulse Code Modulation Frame Sync | Wird für die Audioschnittstelle verwendet. |
| TXD | Transmit Data | Sendet serielle Daten. |
| RXD | Recieve Data | Empfängt serielle Daten. |
| PCM_CLK | Pulse Code Modulation Clock | Takt für die Audioschnittstelle. |
| CE0 | Chip Enable 0 | Aktiviert den ersten SPI (Serial Peripheral Interface) Slave. |
| CE1 | Chip Enable 1 | Aktiviert den zweiten SPI Slave. |
| ID_SC | ID for SPI | Wird für die automatische Erkennung von HATs verwendet, die SPI verwenden. |
| PWM0 | Pulse Width Modulation | Ein weiterer Pin für Pulsweitenmodulation. |
| PCM_DIN | Pulse Code Modulation Data In  | Datenleitung für die Audioschnittstelle (Input) |
| PCM_DOUT | Pulse Code Modulation Data Out | Datenleitung für die Audioschnittstelle (Output) |
| 3V3 Power | 3,3 Volt Stromversorgung | 3,3 Volt Stromversorgung |
| 5V Power | 5 Volt Stromversorgung | 5 Volt Stromversorgung |
| GND | Ground | Globaler Bezugspunkt für elektrische Spannung (”Erdet den Pi (sehr wichtig)). |
